...
contract AdvancedTradeBot is ReentrancyGuard, Ownable {
    ...
    *******************/
    ...
    function deposit() payable external onlyUnlocked {
        require(msg.value >= minDeposit, "Deposit value needs to be greater than min deposit amount");
        ...
        totalContributeInRound[currentRound] = totalContributeInRound[currentRound].add(msg.value);
        userContributesInRound[msg.sender][currentRound] = userContributesInRound[msg.sender][currentRound].add(msg.value);
    ...
    function withdraw(uint _amt) external onlyUnlocked nonReentrant {
        ...
        // If the user's deposit amount < 0.000001 BNB, then he/she will be no longer a participant
    ...
    ********************/
    ...
    function refund() payable external onlyOwner onlyLocked {
        
        require(msg.value >= totalContributeInRound[currentRound], "Make sure input amount is larger than total contribute"); 
        
        totalProfitInRound[currentRound] = msg.value.sub(totalContributeInRound[currentRound]);
        ...
        currentRound ++;
        isLocked = false;
        emit refunded(currentRound.sub(1), msg.value, block.timestamp);
    }
